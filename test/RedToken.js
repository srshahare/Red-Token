...
contract("RedToken", function (accounts) {
  let tokenInstance;

  it("initializes the contract with the currect values", function () {
    return RedToken.deployed()
      ...
      .then(function (name) {
        assert.equal(name, "Red Token", "has the correct name");
        return tokenInstance.symbol();
      })
      .then(function (symbol) {
        assert.equal(symbol, "RED", "has the correct symbol");
        return tokenInstance.standard();
      })
      .then(function (standard) {
        assert.equal(standard, "Red Token v1.0", "has the correct standard");
      });
  });
    ...
    return RedToken.deployed()
      ...
      .then(function (receipt) {
        ...
        assert.equal(
          ...
          accounts[0],
          "logs the account the tokens are tranferred from"
        );
        assert.equal(
  ...
  it('handles delegated token transfers', function() {
    return RedToken.deployed().then(function(instance) {
      tokenInstance = instance;
